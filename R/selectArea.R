...
# The format is (assume the returned object is called ``lt``):
#
...
selectArea = function(ht_list, pos1 = NULL, pos2 = NULL, mark = TRUE, verbose = TRUE,
	...
	if(is.null(ht_pos)) ht_pos = ht_pos_on_device(ht_list)
	ht_pos = lapply(ht_pos, function(x) lapply(x, function(y) {
		list(x = convertX(y$x, unit, valueOnly = TRUE),
		     y = convertY(y$y, unit, valueOnly = TRUE))
	}))
...
# The format is (assume the returned object is called ``lt``):
#
# - ``lt[[ heatmap_name ]][[ slice_name ]][[ row_index ]]``: row index of the selected point.
# - ``lt[[ heatmap_name ]][[ slice_name ]][[ column_index ]]``: column index of the selected point.
#
# == author
# Zuguang Gu <z.gu@dkfz.de>
#
	...
	for(ht_name in names(ht_pos)) {
		...
		for(vp_name in names(ht_pos[[ht_name]])) {
			...
			if(ind1 <= 0 || ind1 > nc) { # the region is on the left of the heatmap
				...
				res[[ht_name]][[vp_name]]$column_index = ht@column_order_list[[j]][ind1]
			}
			...
			if(ind1 <= 0  || ind1 > nr) { # the region is on the bottom of the heatmap
				...
				res[[ht_name]][[vp_name]]$row_index = ht@row_order_list[[i]][ind1]
			}
