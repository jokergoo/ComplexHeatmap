...
# -gap If legend grids are put into multiple columns, this controls the gap between neighbouring columns, measured as a `grid::unit` object. Overruled by the col_gap parameter
# -col_gap If legend grids are put into multiple columns, this controls the gap between neighbouring columns, measured as a `grid::unit` object. This parameter overrules the gap parameter, which is still included for backward compatibility.
# -row_gap If legend grids are put into multiple rows, this controls the gap between neighbouring rows, measured as a `grid::unit` object.
...
Legend = function(at, labels = at, col_fun, nrow = NULL, ncol = 1, by_row = FALSE,
  ...
  if(missing(col_fun)) {
    ...
    legend_body = discrete_legend_body(at = at, labels = labels, 
                                       nrow = nrow, ncol = ncol,
                                       ...
                                       pch = pch, size = size, 
                                       by_row = by_row, 
                                       cell_scaling_factors = cell_scaling_factors)
  } else {
...
discrete_legend_body = function(at, labels = at, nrow = NULL, ncol = 1, 
  by_row = TRUE, grid_height = unit(4, "mm"), grid_width = unit(4, "mm"), 
  col_gap = unit(2, "mm"),
  row_gap = unit(0, "mm"),
  labels_gp = gpar(fontsize = 10),
  border = "white", background = "#EEEEEE",
  type = "grid", legend_gp = gpar(), pch = 16, size = unit(2, "mm"), 
  cell_scaling_factors = 1) {
  ...
  labels_mat = matrix(c(labels, rep("", nrow*ncol - n_labels)), 
                      nrow = nrow, ncol = ncol, byrow = by_row)
  ...
  legend_body_width = grid_width*ncol + labels_padding_left*ncol + 
    sum(labels_max_width)
  legend_body_height = nrow*(grid_height+row_gap)
  ...
  for(i in 1:ncol) {
    ...
    ni = length(index)
    y = (0:(ni - 1)) * (grid_height + row_gap)
    y = legend_body_height - y

    labels_x = grid_width*i + sum(labels_max_width[1:i]) + 
      labels_padding_left*i - labels_max_width[i]
    labels_y = y - grid_height*0.5
    labels_x = convertWidth(labels_x, "mm")
    labels_y = convertHeight(labels_y, "mm")
    gl = c(gl, list(
      textGrob(labels[index], x = labels_x, y = labels_y, 
               just = "left", gp = labels_gp)
    ))
    ...
    grid_x = grid_width*i + sum(labels_max_width[1:i]) + 
      labels_padding_left*i - labels_max_width[i] - 
      labels_padding_left - grid_width*0.5
    ...
    gl = c(gl, list(
      rectGrob(x = grid_x, y = grid_y, 
        width = sqrt(cell_scaling_factors) * grid_width, 
        height = sqrt(cell_scaling_factors) * grid_height, gp = sgd2)
    ))
