...
Define the colors. In most cases, the heatmap visualizes a matrix which maps continuous values
...
a named vector so that it is possible to map discrete values to colors.

```{r}
mat2 = matrix(sample(1:4, 1000, replace = TRUE), 10, 10)
...
```{r}
mat2 = matrix(sample(letters[1:4], 1000, replace = TRUE), 10, 10)
colors = structure(rand_color(4), names = letters[1:4])
Heatmap(mat2, col = colors)
```

The name of the heatmap is used for the title of the heatmap legend.
The name is also useful if you plot more than one heatmap (introduced laster).
...
Cluster settings. There are three ways to specify distance metric for clustering:

- specify distance as a pre-defined option. The valid values are the supported methods
  in `dist()` function and `pearson`, `spearman` and `kendall`.
- a self-defined function which calculate distance from a matrix. The function should
...
Heatmap(mat, name = "test", row_hclust_width = unit(1, "cm"))
```
...
ht = Heatmap(mat, rect_gp = gpar(col = "white"), 
  cell_fun = function(i, j, x, y, width, height) grid.text(mat[i, j], x = x, y = y),
