...
%\VignetteIndexEntry{Heatmap and Annotation Legends}
...
You can choose to only show some of the heatmap legends by setting `show_heatmap_legend` to a logical value.
...
ha_column = HeatmapAnnotation(df = data.frame(type1 = c(rep("a", 5), rep("b", 5))), 
    col = list(type = c("a" =  "red", "b" = "blue")), show_legend = FALSE)
ha_row = rowAnnotation(df = data.frame(type2 = c(rep("A", 4), rep("B", 4))), 
    col = list(type = c("A" =  "green", "B" = "orange")), show_legend = FALSE, width = unit(1, "cm"))
...
styles are still limited. You can construct a legend grob totally from ground by `frameGrob()`
and `packGrob()` but it is too high level for most of the users.
